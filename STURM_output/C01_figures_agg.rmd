...
var <- "heat_kWh"
ref <- "stock_building"
path <- "_consumption_hh_countries.png"
make_plot_lines(data, var, ref, path)

var <- "heat_kWh"
ref <- "floor_m2"
path <- "_consumption_m2_countries.png"
make_plot_lines(data, var, ref, path)
```
...
plot_lines(temp,
          ...
          subplot = TRUE,
          free_y = TRUE,
          save_path = paste(save_dir, paste0(scenario, "_stock_housing_agg_countries.png"), sep = "/"))
...
plot_lines(temp,
          ...
          subplot = TRUE,
          free_y = TRUE,
          save_path = paste(save_dir, paste0(scenario, "_consumption_agg_countries.png"), sep = "/"))
...
plot_lines(temp,
          ...
          subplot = TRUE,
          free_y = TRUE,
          save_path = paste(save_dir, paste0(scenario, "_renovation_agg_countries.png"), sep = "/"))

```

### Unit space heating energy consumption
```{r}
temp <- data %>%
  filter(variable == "stock_building") %>%
...
plot_stacked_areas(temp, "year", "value", "resolution", "region_bld",
                    y_label = "Stock housing units",
                    save_path = paste(save_dir, paste0(scenario, "_stock_fuels_countries.png"),
                    sep = "/"))
...
plot_stacked_areas(temp, "year", "value", "resolution", "region_bld",
                    y_label = "Stock housing units\nMillion",
                    save_path = paste(save_dir, paste0(scenario, "_consumption_fuels_countries.png"),
                    sep = "/"))
```