...
if (run_test) {
      ...
      scenarios <- c(
        ...
        "EU_price_constant_exogenous" = "Price constant",
        "EU_price_2020_600_exogenous" = "Carbon tax",
        "EU_double_exogenous" = "Doubling renovation",
        "EU_double_price_constant_exogenous" =
          "Doubling renovation, prices constant",
...
colors_scenarios <- c(
  ...
  "Doubling renovation, prices constant" = "#531717",
...
variables <- c("heat_kWh", "heat_std_kWh", "heat_tCO2",
     "energy_poverty_thres", "stock_building", "heating_intensity")
...
temp <- data %>%
  filter(variable == "stock_building") %>%
  filter(year == 2050) %>%
  filter(resolution %in% c("biomass_solid", "heat_pump",
    "electricity", "district_heating")) %>%
  group_by_at(c("region_bld", "scenario")) %>%
...
format_temp <- table %>%
  ...
  select(c("region_bld", "scenario",
    ...
    "heating_intensity",
    "stock_building", "low_carbon_building",
    "renovated_building", "energy_poverty_thres")) %>%
...
write.csv(format_temp, paste(save_dir, paste0(run, "_summary.csv"), sep = "/"),
  row.names = FALSE)
```
