...
run_scenario <- function(run, 
  ...
  if(input_mode== "csv") {
    ...
    # Source - input data
    d <- fun_inputs_csv(path_in, path_inputs, file_inputs, sector, run)
  ...
  if (sector == "resid") {
    ...
    if (mod_vacant == "vacant") { # V:
      ...
      print("DEBUG key objects")
      print(is.null(d$pop))
      print(is.null(d$pop_urt))
      print(is.null(d$pop_clim))
      print(is.null(d$hh_size))
      print(is.null(d$floor_cap))
      print(is.null(d$stock_vacant_base))
      print(is.null(d$shr_vacant_base_arch))
      print(is.null(d$shr_vacant_base_period))
      ...
      lst_stock_init <- fun_stock_init_fut(sector,
        ...
        lst_stock_init <- fun_stock_init_fut(sector,
                                             ...
                                             d$stock_arch_base,
                                             d$shr_mat, d$shr_arch, d$shr_fuel_heat_base, d$shr_distr_heat,
                                             stock_vacant_base = if (mod_vacant == "vacant") d$stock_vacant_base else NULL,
                                             shr_vacant_base_arch = if (mod_vacant == "vacant") d$shr_vacant_base_arch else NULL,
                                             shr_vacant_base_period = if (mod_vacant == "vacant") d$shr_vacant_base_period else NULL,
                                             d$en_int_heat, d$en_int_cool,
                                             d$days_cool,
                                             d$eff_cool, d$eff_heat,
                                             d$en_sav_ren, 
    ...
    # Extract dataframes from initialization list
    ...
    if (mod_vacant == "vacant") {
      stock_vacant_i = lst_stock_init$stock_vacant_i
    }
    ...
    for (i in 2:length(yrs)) {
      ...
      # Renovation and fuel-switch decisions
      ...
      if(mod_ren == "endogenous"){
        ...
        # F05: renovation changes efficiency only; heating fuel is retained.
        ...
        relative_preference_fuel_noren_zhu <- fun_sw_pref_zhu(yrs,i,
        ...
        # F06: calculate probabilities from the stock at period start.
      ...
      if (mod_ren == "zhu") {
        ...
        try(if(nrow(ms_ren_i_zhu) == 0) {
          stop("Error in renovation calculation! Empty dataframe ms_ren_i_zhu")
        })

        # F06 sequential branch: apply both decisions and complete the stock
        # update using the same public function as the original modes.
        lst_stock_i <- fun_stock_dyn(sector,
                                     ...
                                     d$prob_dem,
                                     stock_vacant_i = if (mod_vacant == "vacant") stock_vacant_i else NULL,
                                     rate_vacant_occ = if (mod_vacant == "vacant") d$rate_vacant_occ else NULL,
                                     rate_switch_fuel_heat = NULL, # not used in zhu mode
                                     ms_new_i, ms_ren_i_zhu, rate_ren_i,
                                     ms_sw_i = NULL, # not used in zhu mode
      ...
      } else if (mod_vacant == "vacant") {

        lst_stock_i <- fun_stock_dyn(
          sector, mod_arch, mod_vacant, yrs, i, run,
          geo_level, geo_level_aggr, geo_levels,
          d$bld_cases_fuel, d$bld_cases_eneff,
          d$ct_bld_age, d$ct_fuel_comb,
          d$hh_size, d$floor_cap,
          stock_aggr, bld_det_age_i,
          d$prob_dem,
          stock_vacant_i, d$rate_vacant_occ,
  ...
  if (sector == "comm") {
    ...
    for (i in 2:length(yrs)) {
      ...
      if (mod_vacant == "vacant") {
        stock_vacant_i = lst_stock_i$stock_vacant_i
      }
