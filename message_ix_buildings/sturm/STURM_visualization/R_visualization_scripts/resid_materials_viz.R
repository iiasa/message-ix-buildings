# ============================================================
# STURM Results — Residential Material Demand Panels
#
...
#
# Uses:
#   *_resid_region_bld_material.csv
# ============================================================
...
make_material_file_path <- function(scenario) {
  file.path(output_dir,
            paste0("report_STURM_", scenario, "_resid_region_bld_material.csv"))
}
...
# Residential has brick — add to material colors
material_colors <- c(
  "aluminum" = "#D95F02",
  "brick"    = "#FF1F1F",
  "cement"   = "#969696",
...
pA_global_mat <- df_plot_unique %>%
  ...
  labs(
    title    = sprintf("Global residential material demand — %d unique trajectories", n_groups),
    subtitle = "Annual residential building material demand | scenarios with identical projections grouped",
...
ggsave(file.path(plot_dir, "plot_02A_resid_global_material_demand.png"),
...
pB_region_mat_2100 <- df_pct_2100 %>%
  ...
  labs(
    title    = paste0("Regional residential material demand — % breakdown by material, ", panel_b_year),
...
ggsave(file.path(plot_dir, "plot_02B_resid_regional_material_pct_2100.png"),
...
###############
# ── Step 13: Panel C — Narrow vs Reference focus (R, N_r, N_tp only) ─────────
...
pC_narrow_focus <- df_pct_2100_focus %>%
  ...
  labs(
    ...
    subtitle = "R = Reference | N_r = Narrow moderate | N_tp = Narrow ambitious | wood share increases under N_tp",
    x = NULL,
...
ggsave(file.path(plot_dir, "plot_02C_resid_material_pct_narrow_focus.png"),
       plot = pC_narrow_focus, width = 13, height = 9, dpi = 300)

message("✓ Narrow focus material plot saved")
