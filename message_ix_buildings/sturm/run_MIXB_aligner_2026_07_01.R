...
ALIGN_COMMODITIES <- list(
  ...
  biomass = c(
    "biomass_resid_heat", "biomass_resid_hotwater","biomass_resid_cook",
    "biomass_comm_heat", "biomass_comm_hotwater"
  ),
  d_heat = c(
    "d_heat_resid_heat", "d_heat_resid_hotwater",
    "d_heat_comm_heat", "d_heat_comm_hotwater"
  ),
  electricity = c(
...
IEA_VARIABLE_FUEL <- c(
  ...
  "Final Energy|Residential and Commercial|Solids|Biomass"= "biomass",
  "Final Energy|Residential and Commercial|Heat"          = "d_heat"
)
...
align_commodity_to_mixb <- function(align_commodity) {
  m <- stringr::str_match(align_commodity, "^(.*?)_(resid|comm)_(.+)$")
  if (is.na(m[1, 1])) {
